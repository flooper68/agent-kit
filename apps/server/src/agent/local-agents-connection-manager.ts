...
/**
 * Manages local agent connection state in Redis and publishes connection status events.
 */
export class LocalAgentsConnectionManager {
  ...
  private getUserConnectionsKey(userId: string): string {
    return `local-agents:connected:${userId}`;
  }
  ...
  private getStatusChannel(userId: string): string {
    return `local-agents:status:${userId}`;
  }

  /**
   * Register a new local agent connection
   */
  ...
  /**
   * Unregister a local agent connection
   */
  ...
  async getConnectionStatus(
    userId: string,
    agentId: string
  ): Promise<LocalAgentConnection | null> {
   ...
   */
  async getUserConnections(userId: string): Promise<LocalAgentConnection[]> {
    const data = await this.redis.hgetall(this.getUserConnectionsKey(userId));

    const connections: LocalAgentConnection[] = [];
  ...
  async cleanupAllConnections(): Promise<void> {
    ...
    const pattern = 'local-agents:connected:*';
    const keys: string[] = [];
