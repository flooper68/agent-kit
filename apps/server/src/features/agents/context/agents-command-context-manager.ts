...
import type { AgentsCommandContext } from './types';
 ...
 */
export class AgentsCommandContextManager {
  constructor(
   ...
   *   await cacheInvalidation?.publishAgentCreated(userId, created.id);
   *   return created;
   * });
   * ```
   */
  handleCommand = async <T>(
    fn: (ctx: AgentsCommandContext) => Promise<T>
  ): Promise<T> => {
