...
A planning and brainstorming assistant powered by Claude Opus. Helps users explore ideas, organize projects, and manage tasks within Agent Kit.
...
```bash
cd apps/local-agent
export CLAUDE_ASSISTANT_OPUS_AGENT_API_KEY=your_key
...
# Build and run
docker compose build claude-assistant-opus
docker compose up claude-assistant-opus
```
...
docker run -it \
  -e CLAUDE_ASSISTANT_OPUS_AGENT_API_KEY=your_key \
